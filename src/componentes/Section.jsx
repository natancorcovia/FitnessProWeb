    ...
    <div>
        <section className={`flex flex-col-reverse md:flex-row items-center justify-between px-4 md:px-10 gap-10 ${reverse ? 'md:flex-row-reverse' : ''}`}>
            <div className="text-white font-[Poppins] w-full md:max-w-xl" >
                ...
                <h2 className="font-bold text-[#5C5A5A] text-base md:text-xl">{subtitle}</h2>
                <h1 className="text-4xl md:text-6xl lg:text-5xl font-bold my-6">{title}</h1>
                <p className="text-[#5C5A5A] text-xs md:text-lg">{description}</p>
